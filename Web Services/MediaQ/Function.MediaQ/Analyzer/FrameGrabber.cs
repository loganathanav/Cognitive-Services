...
namespace Function.MediaQ.Analyzer
{
    public class FrameGrabber<AnalysisResultType>
    {
        ...
        public string strStreamingUrl { get; set; }
        ...
        public FrameGrabber(JObject mediaObject, ZetronDbContext dbContext, TraceWriter log)
        {
            _mediaObject = mediaObject;
            _dbContext = dbContext;
            _log = log;
            strStreamingUrl = _mediaObject["mediaUrl"].ToString();
        }
        ...
        public async Task StartProcessingAsync(double overrideFPS = 0)
        {
            _log.Info($"Clearning task queue.");
            await StopProcessingAsync().ConfigureAwait(false);

            _reader = new VideoCapture(strStreamingUrl);
        ...
        {
            _log.Info($"Inside StartProcessing...");
                ...
                while (!_stopping)
                {
                    ...
                    if (!success)
                    {
                        ...
                        if (_reader.CaptureType == CaptureType.File)
                        {
                            ...
                            // Break out of the loop to make sure we don't try grabbing more frames. 
                            break;
                        }
                        else
                        {
        ...
        /// <summary>
        /// Check the status of the live streaming
        /// </summary>
        ...
        public bool CheckStreamingStatus()
        {
            ...
            {
                _log.Error($"Video processing exception.: ", e);
            }
            return false;
        }
